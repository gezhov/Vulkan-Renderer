...
namespace vget
{
    ...
    /// <summary>
    /// Функция выбора режима показа кадров (Present Mode).
    /// </summary>
    /// <param name="availablePresentModes">Вектор поддерживаемых режимов показа VkPresentModeKHR</param>
    /// <returns>Выбранный режим показа VkPresentModeKHR</returns>
    VkPresentModeKHR VgetSwapChain::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes)
    {
	    ...
	    /* Резервный вариант в виде режима показа Immediate. В этом режиме
	       вертикальная синхронизация игнорируется. */
	     /*for (const auto &availablePresentMode : availablePresentModes) {
    ...
    VkExtent2D VgetSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
    {
